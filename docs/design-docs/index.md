...
- `ARCHITECTURE.md` と `docs/DOCUMENTATION_SYSTEM.md` から段階的にたどる詳細設計の索引として使う。

運用ルール
- 各ファイルは「何を決めたか」だけでなく「なぜそうしたか」を残す。
- 追加・削除時は `python scripts/doc_gardening.py` を実行して索引を同期する。
